...
export function ProfileDetail({ slug }: Props) {
  ...
  if (!student) return <ProfileNotFound />;
        ...
        <article className="grid grid-cols-[200px_1fr] items-start gap-10 pb-20 max-[720px]:grid-cols-1 max-[720px]:gap-6">
          ...
          <div>
            {student.openTo.length > 0 && (
              <div className="mb-4 flex flex-wrap gap-1.5">
                {student.openTo.map((id, i) => (
                  <span key={id} className="inline-flex items-center gap-1.5 rounded-full border border-line-2 bg-[#f0eee2] px-3 py-1.25 font-mono text-xs font-medium tracking-[0.02em] text-fg whitespace-nowrap">
                    {i === 0 && (
            ...
            <div className="mb-6 text-[15px] leading-normal text-muted">
              <strong className="font-medium text-fg">{student.university}</strong>
              <br />
