...
export function ProfileContact({
  ...
  const hasCv = Boolean(cvName);

  if (!hasContact && !hasCv) return null;

  const href = hasContact ? formatContact(contactType, contact) : "#";
      ...
      {hasCv && (
        <>
          <button
            ...
            className="flex items-center gap-2.5 rounded-[14px] border border-line-2 bg-white px-4.5 py-3.5 text-sm font-medium text-fg transition-[background,border-color,transform] duration-150 hover:-translate-y-px hover:border-fg hover:bg-[#f8f6ee] disabled:opacity-60"
          >
            <svg
          ...
          {cvDownloadError && (
            <div className="px-1 font-mono text-[11px] tracking-[0.02em] text-danger">
              ↳ {cvDownloadError}
            </div>
          )}
        </>
